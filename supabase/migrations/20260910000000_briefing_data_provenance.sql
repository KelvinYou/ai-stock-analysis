-- Expose `briefing_data_as_of`: the newest price bar the briefing's analysis
-- actually read.
--
-- `briefing_date` records when the run happened, not what it saw, so it cannot
-- distinguish a briefing that is merely a few days old from one whose signal
-- was formed on a materially older tape. The screener compares this column
-- against `market_as_of_date` to flag the latter — a `buy` adjudicated on a
-- $592.85 close was served for a week beside a $617.79 price with no warning.
--
-- `create or replace view` requires the existing columns to keep their names,
-- types and order, so the whole select is restated with the new column
-- appended at the end.
...
select
    ...
    (b.payload->>'data_as_of') as briefing_data_as_of
